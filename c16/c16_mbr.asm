	...
	;|--------------------------------------|-------------------------------------|
         ...
         ;建立#3保护模式下的堆栈段描述符      ;基地址为0x00007C00，界限0xFFFFE 
  ...
  flush:     
	...
	;|------------------------|---|------|
	;|             0008                  |
         mov eax,0x0008                     ;加载数据段(0..4GB)选择子,ds指向数据段
         mov ds,eax
 ...
 setup:
         mov esi,[0x7c00+pgdt+0x02]         ;不可以在代码段内寻址pgdt，但可以
                                            ;通过4GB的段来访问
         ;重新构建描述符#1建立公用例程段描述符,edi内核头部的地址
         mov eax,[edi+0x04]                 ;公用例程代码段起始汇编地址
         mov ebx,[edi+0x08]                 ;核心数据段汇编地址
         sub ebx,eax			    ;得到段的总长度
         dec ebx                            ;公用例程段界限,段长度-1，就是段界限
         add eax,edi                        ;公用例程段基地址 eax = 0x40000 + eax，真正段的加载地址
         mov ecx,0x00409800                 ;字节粒度的代码段描述符 ecx 表示属性的一部分
         call make_gdt_descriptor
         mov [esi+0x28],eax		    ;gdt的低32位
         mov [esi+0x2c],edx		    ;gdt的高32位
...
make_gdt_descriptor:                     ;构造描述符
         ...
         ;dec ebx                            ;公用例程段界限 ebx-eax+1
         ;add eax,edi                        ;公用例程段基地址 eax = 0x40000 + eax
         ;mov ecx,0x00409800                 ;字节粒度的代码段描述符 ecx 表示属性的一部分
         ...
         xor bx,bx			 ;清楚ebx的低16位，界限是20位，留下的就是limit的19-16位	
